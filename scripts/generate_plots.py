...
def _size_label(s) -> str:
    """Human-readable label for an input-size value (int, float, or string)."""
    try:
        v = float(s)
        return f"{v:.0e}" if v >= 1e6 else str(int(v))
    except (ValueError, TypeError):
        return str(s)
...
def plot_grouped_bar_cpu(df: pd.DataFrame, output_dir: Path):
    ...
    for idx, bench in enumerate(benchmarks):
        ...
        ax.set_xticklabels([_size_label(s) for s in sizes])
        ax.legend()
        ax.set_yscale('log')
...
def plot_gpu_comparison(df: pd.DataFrame, output_dir: Path):
    ...
    for idx, bench in enumerate(benchmarks):
        ...
        ax.set_title(bench, fontweight='bold')
        ax.set_xticks(x + width / 2)
        ax.set_xticklabels([_size_label(s) for s in sizes], rotation=30)
        ax.legend()
...
def plot_speedup_heatmap(df: pd.DataFrame, output_dir: Path):
    ...
    all_sizes = sorted(df['input_size'].unique(), key=lambda s: (0, float(s)) if _is_numeric(s) else (1, str(s)))
    ...
    for bench in benchmarks:
        ...
        sizes = sorted(bench_df['input_size'].unique(),
                       key=lambda s: (0, float(s)) if _is_numeric(s) else (1, str(s)))
        for size in sizes:
            ...
            row.append(ratio)
            size_labels.add(_size_label(size))
        matrix.append(row)
...
def plot_violin_distributions(df: pd.DataFrame, output_dir: Path):
    ...
    for idx, bench in enumerate(benchmarks):
        ...
        sns.violinplot(data=data, x='language', y='time_ms', ax=ax,
                       hue='language', palette=palette, legend=False,
                       inner='box', cut=0)

        ax.set_title(f'{bench} (n={_size_label(largest_size)})', fontweight='bold')
        ax.set_xlabel('')
        ax.set_ylabel('Time (ms)')
...
def load_summary_as_trials(csv_path: Path, n_synthetic: int = 30) -> pd.DataFrame:
    """
    Reshape analysis_summary.csv (wide format: cpp_mean_ms, rust_mean_ms, …)
    into the long trial-level format the plot functions expect:
    columns: benchmark, language, input_size, time_ms, is_gpu

    Synthetic trial distributions are generated from the stored mean ± std
    so that error bars and violin plots reflect real variability.
    """
    ...
    for _, row in raw.iterrows():
        is_gpu = "cuda" in str(row["benchmark"]).lower()
        # Try to convert input_size to a number so sorting/formatting works in plots
        raw_size = row["input_size"]
        try:
            input_size = float(str(raw_size).replace(",", ""))
        except ValueError:
            input_size = raw_size  # keep as string for composite sizes like "64x16384"

        for lang, mean_col, std_col in [
            ("C++",  "cpp_mean_ms",  "cpp_std_ms"),
            ("Rust", "rust_mean_ms", "rust_std_ms"),
        ]:
            mean = float(row[mean_col])
            std  = float(row[std_col]) if std_col in row.index else mean * 0.02
            std  = max(std, 1e-9)
            trials = rng.normal(mean, std, size=n_synthetic)
...
def main():
    ...
    if args.sample:
        ...
        print("  Using generated sample data (--sample flag)")
    else:
        ...
        if summary_csv.exists():
            df = load_summary_as_trials(summary_csv)
            print(f"  Loaded real benchmark data from {summary_csv.name}")
        else:
            df = generate_sample_data()
            print("  ⚠ analysis_summary.csv not found — using sample data")

...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{C++ compiler:} GCC via CMake Release build
    (\texttt{-O3 -DNDEBUG}); CUDA kernels compiled with \texttt{nvcc -O3}
  \item \textbf{Rust compiler:} \texttt{rustc} stable channel,
    \texttt{cargo build -{}-release} (\texttt{opt-level = 3})
  \item \textbf{CPU benchmark harness:} Google Benchmark (C++) and
    Criterion.rs (Rust); inputs defined by
...
First, the GPU benchmark set previously contained only five trials, which
yielded high variance; though expanded to 30 trials for this report,
hardware-specific latency outliers may still influence the distributions.
...
The standard-library sort advantage ($-$80.6\,\%) explicitly
reflects algorithmic differences between C++ \texttt{std::sort} (introsort) and
Rust's pattern-defeating quicksort (pdqsort), rather than a pure language-level
performance difference. (Manual implementation comparisons isolate this).
...
excluded.
Note that the GPU benchmarks initially exhibited high CV\,\% values due to a
small trial count in preliminary runs; re-evaluating at $n=30$ improves
statistical reliability.

\begin{table}[htbp]
\caption{GPU kernel performance: C++ vs.\ Rust (kernel time only,
         H2D/D2H transfer excluded, $n=30$ trials per benchmark).}
...
difference. Both previously exhibited very high CV\,\% values, which may reflect warm-up
variability in first-call kernel launch latency.
While a larger trial count ($n=30$) provides more stable point estimates, further
profiling is necessary to analyze launch overheads definitively.
...
The matmul and reduction outcomes favour Rust, but initially exhibited high
CV\,\%, requiring larger trial counts.
...
section: with high variance latency layers, no firm conclusion about
relative execution phase performance is fully isolated.
...
The GPU evaluation depends heavily on driver implementation and driver latency;
while trial counts improved stability, driver-induced timing noise can remain.
